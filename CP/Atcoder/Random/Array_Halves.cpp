...
struct fenwick_tree {
    ...
    fenwick_tree(int64_t n) : size(n + 2) { ft.assign(size, 0); }
...
void solve() {
    int64_t n = 0L;
    std::cin >> n;
    std::vector<int64_t> p(2 * n);
    read_vec(0, 2 * n, p);

    std::vector<int64_t> binary(2 * n);
    ...
    int64_t large_nums_seen = 0;
    for (int64_t i = 0; i < 2 * n; ++i) {
        if (binary[i] == 1) {
            large_nums_seen++;
        } else {
            inversions += large_nums_seen;
        }
    }
